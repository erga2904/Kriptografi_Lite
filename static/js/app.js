...
function switchTab(cipher) {
    ...
    const mobSelect = document.getElementById('mobile-cipher-select');
    if (mobSelect) mobSelect.value = cipher;

    tabs.forEach((t, idx) => {
        const el = document.getElementById(`tab-${t}`);
        if (!el) return; // Guard for mobile where tabs might be hidden but IDs still exist
        if (t === cipher) {
...
function highlightVigenereCell(rowIdx, colIdx, stepInfo) {
    ...
    if (target) {
        ...
        const alreadyActive = overlay.classList.contains('active');
        ...
        if (container) {
            setTimeout(() => {
                ...
                setTimeout(() => {
                    const floatWin = document.getElementById('floatingSteps');
                    const showBtn = document.getElementById('showStepsBtn');
...
function toggleFloatingSteps() {
    ...
    if (floatWin.classList.contains('hidden')) {
        // Tampilkan kembali
        floatWin.classList.remove('hidden');
        setTimeout(() => floatWin.classList.add('visible'), 10);
        showBtn.classList.add('hidden');
    } else {
        // Sembunyikan
        floatWin.classList.remove('visible');

        // Sesuaikan posisi tombol "Restore" dengan posisi jendela saat ini
        showBtn.style.left = floatWin.style.left;
        showBtn.style.right = floatWin.style.right;
        showBtn.style.bottom = floatWin.style.bottom;
...
async function process(mode) {
    ...
    if (currentCipher === 'affine') {
        payload.a = document.getElementById('inputA').value;
        payload.b = document.getElementById('inputB').value;
        ...
        payload.e = document.getElementById('rsa_e').value;
        payload.d = document.getElementById('rsa_d').value;
        payload.n = document.getElementById('rsa_n').value;
    } else {
        payload.key = key;
    }

    if (!text || (currentCipher !== 'rsa' && !key && currentCipher !== 'affine' && currentCipher !== 'hill')) {
    ...
    try {
        ...
        if (data.error) {
            showAppWarning(data.error, 'Kesalahan Proses');
            return;
        }
        ...
        addToHistory(currentCipher, mode, text, data.result);

        // Trigger visualization for Vigenere
        if (data.viz_steps && data.viz_steps.length > 0) {
            currentVizSteps = data.viz_steps;
            currentVizStepIdx = 0;
            showVizStep(0);
        }
    } catch (err) {
        showAppWarning(err.message, 'Kesalahan Koneksi');
    }
}

let currentVizSteps = [];
let currentVizStepIdx = 0;

function showVizStep(idx) {
    if (idx < 0 || idx >= currentVizSteps.length) return;
    currentVizStepIdx = idx;
    const step = currentVizSteps[idx];
    
    // Add navigation buttons to the info HTML
    let navHtml = `
        <div class="flex items-center justify-between gap-2 mt-4 pt-4 border-t border-white/10">
            <button onclick="showVizStep(${idx - 1})" ${idx === 0 ? 'disabled' : ''} class="flex-1 py-2 bg-white/5 hover:bg-white/10 disabled:opacity-30 disabled:cursor-not-allowed rounded-lg text-xs font-bold transition-all border border-white/5 active:scale-95 flex items-center justify-center gap-1">
                <svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg>
                Prev
            </button>
            <span class="text-[10px] text-gray-500 font-mono">${idx + 1} / ${currentVizSteps.length}</span>
            <button onclick="showVizStep(${idx + 1})" ${idx === currentVizSteps.length - 1 ? 'disabled' : ''} class="flex-1 py-2 bg-indigo-600 hover:bg-indigo-500 disabled:opacity-30 disabled:cursor-not-allowed rounded-lg text-xs font-bold text-white transition-all border border-indigo-500/20 active:scale-95 flex items-center justify-center gap-1">
                Next
    ...
    highlightVigenereCell(step.row, step.col, step.info + navHtml);
}

function fillExampleData(mode = 'encrypt') {
    const textElem = document.getElementById('inputText');
    const keyElem = document.getElementById('inputKey');
    
    if (mode === 'encrypt') {
        textElem.value = "CIPHERLAB";
    } else {
        // Different dummy ciphertext for decryption mode
        if (currentCipher === 'rsa') {
            textElem.value = "18 24 37";
        } else if (currentCipher === 'vigenere') {
            textElem.value = "MSZREBVKG";
        } else if (currentCipher === 'caesar') {
            textElem.value = "FLSKHUODE";
        } else if (currentCipher === 'hill') {
            textElem.value = "KZZIIVHGF";
        } else if (currentCipher === 'playfair') {
            textElem.value = "AM DB RI MZ BX";
        } else {
            textElem.value = "MSZREBVKG";
        }
    }
    ...
    switch(currentCipher) {
        ...
        case 'affine':
            ...
            if (mode === 'decrypt') textElem.value = "IAPZCRLIL";
            break;
        case 'hill':
            document.getElementById('m00').value = "3";
            document.getElementById('m01').value = "3";
            document.getElementById('m10').value = "2";
            document.getElementById('m11').value = "5";
            break;
        case 'playfair':
            keyElem.value = "MONARCHY";
            break;
        case 'rsa':
            document.getElementById('rsa_e').value = "7";
            document.getElementById('rsa_d').value = "23";
            document.getElementById('rsa_n').value = "55";
            break;
    }
}
...
function renderHistory() {
    ...
    if (!list) return;
    if (history.length === 0) {
        list.innerHTML = '<p class="text-center text-gray-500 text-sm py-4">Belum ada riwayat.</p>';
        return;
    }
    list.innerHTML = history.map(item => `
        <div class="flex items-center justify-between p-3 rounded-lg bg-white/5 border border-white/5 hover:bg-white/10 transition-all">
            <div class="space-y-1">
                <div class="flex items-center gap-2">
                    <span class="text-[10px] font-bold uppercase tracking-widest text-indigo-400">${item.algo}</span>
                    <span class="px-1.5 py-0.5 rounded text-[8px] font-bold uppercase ${item.mode === 'encrypt' ? 'bg-green-500/10 text-green-400' : 'bg-blue-500/10 text-blue-400'}">${item.mode}</span>
                </div>
                <div class="text-xs text-gray-400 truncate w-48 sm:w-64">In: ${item.input} &rarr; Out: ${item.output}</div>
            </div>
